...
# SERVO <=> duty cycle, calibrate if needed
MIN_DUTY = 2.9
MAX_DUTY = 97.1
dutyCycle32 = 13
dutyCycle33 = 3
...
def execute_matched_pattern(pattern_id, stream, p):
    ...
    stream.close()
    p.terminate()
    servo_thread = threading.Thread(target=moveServo, args=(True,VIDEO_DURATION-deltaSleepServo))
    sleep(2.5)
...
def jiggleServo():
    ...
    time.sleep(0.3)
    # dutyCycle1
    p_32.ChangeDutyCycle(dutyCycle32-1)
    p_33.ChangeDutyCycle(dutyCycle33+1)
    time.sleep(0.1)
    # dutyCycle1
    p_32.ChangeDutyCycle(dutyCycle32+1)
    p_33.ChangeDutyCycle(dutyCycle33-1)
...
def moveServo(start=True, VIDEO_DURATION=0):
    print('moveServo with mode_start =', start)
    ...
    #   p_32.ChangeDutyCycle(0)     # Changes the pulse width to 3 (so moves the servo)
    #   p_33.ChangeDutyCycle(0)     # Changes the pulse width to 3 (so moves the servo)
    if start and VIDEO_DURATION > 5:
      time.sleep(VIDEO_DURATION-5)
      return moveServo(False, 0)
    return True
