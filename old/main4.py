...
def execute_matched_pattern(pattern_id, stream, p):
    ...
    open_servo_thread = threading.Thread(target=camServo2.openServo(), args=(VIDEO_DURATION,))
    open_servo_thread.start()
    record_thread.join()
    open_servo_thread.join()
    print('sleeping {} - 10s before closing servos'.format(VIDEO_DURATION))
    time.sleep(VIDEO_DURATION-10)
    print('closing servos')
    
    close_servo_thread = threading.Thread(target=camServo2.closeServo(), args=(False,))
    
    close_servo_thread.start()
    close_servo_thread.join()
